<template>
  <div class="w-scroller">
    <ul class="w-scroller-ul" :style="{transform : 'translateY(' + y + 'rem)'}" @touchstart.stop.prevent="ts" @touchmove.stop.prevent="tm"
      @touchend.stop.prevent="te">
      <li v-for="(item, index) in data" :key="index" :class="{on : item == v}">{{ item }}</li>
    </ul>
